...
interface IPolygonAddressRegistry {
    function infinix() external view returns (address);
...
interface IPolygonBundleMarketplace {
...
contract PolygonMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IPolygonAddressRegistry public addressRegistry;
    ...
    function _buyItem(
        ...
        IPolygonBundleMarketplace(addressRegistry.bundleMarketplace())
    ...
    function createOffer(
        ...
        IPolygonAuction auction = IPolygonAuction(addressRegistry.auction());
    ...
    function registerRoyalty(
        ...
        require(_isPolygonNFT(_nftAddress), "invalid nft address");
    ...
    function _isPolygonNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.infinix() == _nftAddress ||
            IPolygonNFTFactory(addressRegistry.factory()).exists(_nftAddress) ||
            IPolygonNFTFactory(addressRegistry.privateFactory()).exists(
                _nftAddress
            ) ||
            IPolygonNFTFactory(addressRegistry.artFactory()).exists(
                _nftAddress
            ) ||
            IPolygonNFTFactory(addressRegistry.privateArtFactory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        IPolygonPriceFeed priceFeed = IPolygonPriceFeed(
    ...
    /**
     @notice Update PolygonAddressRegistry contract
     @dev Only admin
     */
    function updateAddressRegistry(address _registry) external onlyOwner {
        addressRegistry = IPolygonAddressRegistry(_registry);
    }
        ...
        require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    IPolygonTokenRegistry(addressRegistry.tokenRegistry())
                        .enabled(_payToken)),
            "invalid pay token"
        );
    }
