...
const PolygonAddressRegistry = artifacts.require("PolygonAddressRegistry");
const Infinix = artifacts.require("Infinix");
const PolygonAuction = artifacts.require("MockPolygonAuction");
const PolygonMarketplace = artifacts.require("PolygonMarketplace");
const PolygonBundleMarketplace = artifacts.require("PolygonBundleMarketplace");
const PolygonNFTFactory = artifacts.require("PolygonNFTFactory");
const PolygonArtFactory = artifacts.require("PolygonArtFactory");
const PolygonTokenRegistry = artifacts.require("PolygonTokenRegistry");
const PolygonPriceFeed = artifacts.require("PolygonPriceFeed");
...
contract(
  ...
  function ([
    ...
    beforeEach(async function () {
      this.fantomAddressRegistry = await PolygonAddressRegistry.new();
      this.infinix = await Infinix.new(platformFeeRecipient, platformFee);

      this.fantomAuction = await PolygonAuction.new();
      ...
      this.fantomMarketplace = await PolygonMarketplace.new();
      ...
      this.fantomBundleMarketplace = await PolygonBundleMarketplace.new();
      ...
      this.fantomNFTFactory = await PolygonNFTFactory.new(
      ...
      this.fantomTokenRegistry = await PolygonTokenRegistry.new();

      this.mockERC20 = await MockERC20.new("wFTM", "wFTM", ether("1000000"));

      this.fantomTokenRegistry.add(this.mockERC20.address);

      this.fantomPriceFeed = await PolygonPriceFeed.new(
        this.fantomAddressRegistry.address,
        this.mockERC20.address
      );

      this.fantomArtFactory = await PolygonArtFactory.new(
        this.fantomMarketplace.address,
        this.fantomBundleMarketplace.address,
        mintFee,
        platformFeeRecipient,
      ...
      await this.fantomAddressRegistry.updateInfinix(this.infinix.address);
    ...
    describe("Minting and auctioning NFT", function () {
      it("Scenario 1", async function () {
        ...
        let balance = await this.infinix.platformFee();
        console.log(`
        ...
        console.log(`
            Now minting...`);
        let result = await this.infinix.mint(
          artist,
        ...
        await this.infinix.setApprovalForAll(
          this.fantomMarketplace.address,
          true,
          { from: artist }
        );
        ...
        console.log(`
            Buyer approves PolygonMarketplace to transfer up to 50 wFTM`);
        ...
        let nftOwner = await this.infinix.ownerOf(new BN("1"));
        console.log(`
      ...
      it("Scenario 2", async function () {
        ...
        console.log(`
            The artist approves the nft to the market`);
        await this.infinix.setApprovalForAll(this.fantomAuction.address, true, {
          from: artist,
        });
        ...
        console.log(`
            *Event AuctionCreated should be emitted with correct values: 
            nftAddress = ${this.infinix.address}, 
            tokenId = 1, 
            payToken = ${this.mockERC20.address}`);
        ...
        console.log(`
            Bidder1 approves PolygonAuction to transfer up to 50 wFTM`);
        ...
        console.log(`
            Bidder2 approves PolygonAuction to transfer up to 50 wFTM`);
        await this.mockERC20.approve(this.fantomAuction.address, ether("50"), {
          from: bidder2,
        });
        ...
        console.log(`
            Bidder3 approves PolygonAuction to transfer up to 50 wFTM`);
        await this.mockERC20.approve(this.fantomAuction.address, ether("50"), {
          from: bidder3,
        });
      ...
      it("Scenario 3", async function () {
        ...
        result = await this.infinix.mint(
          artist,
        ...
        await this.fantomBundleMarketplace.listItem(
          "mynfts",
          [this.infinix.address, this.infinix.address],
          [new BN("1"), new BN("2")],
          [new BN("1"), new BN("1")],
          this.mockERC20.address,
          ether("20"),
        ...
        expect(listing.nfts[0]).to.be.equal(this.infinix.address);
        expect(listing.nfts[1]).to.be.equal(this.infinix.address);
        ...
        console.log(`
            The buyer approves PolygonBundleMarketplace to transfer up to 50 wFTM`);
        await this.mockERC20.approve(
          this.fantomBundleMarketplace.address,
          ether("50"),
          { from: buyer }
        ...
        let nftOwner = await this.infinix.ownerOf(new BN("1"));
        expect(nftOwner).to.be.equal(buyer);
        nftOwner = await this.infinix.ownerOf(new BN("2"));
        expect(nftOwner).to.be.equal(buyer);
